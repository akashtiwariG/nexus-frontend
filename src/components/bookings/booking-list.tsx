...
export function BookingsList({ onSelectBooking }: BookingsListProps) {
    ...
    try {
      ...
      if (
        ...
        (bookingData.payments &&
          ...
          bookingData.payments.reduce((sum: number, payment: any) => sum + Number(payment.amount), 0) >=
  ...
  if (selectedBookingId && selectedBookingData) {
    ...
    const paymentsCollected =
      selectedBookingData.payments?.reduce((sum: number, payment: any) => sum + (Number(payment.amount) || 0), 0) || 0
            ...
            <div className="bg-white rounded-lg border shadow-sm">
              ...
              <div className="p-6">
                <div className="mb-6">
                  ...
                  <p className="text-gray-500">
                    {selectedBookingData.numberOfGuests || 1}{" "}
                    {selectedBookingData.numberOfGuests === 1 ? "Guest" : "Guests"} ·
                    {selectedBookingData.roomTypeBookings?.[0]?.numberOfRooms || 1}{" "}
                    {selectedBookingData.roomTypeBookings?.[0]?.numberOfRooms === 1 ? "Room" : "Rooms"} ·
                    {calculateNights(selectedBookingData.checkInDate, selectedBookingData.checkOutDate)}{" "}
                    {calculateNights(selectedBookingData.checkInDate, selectedBookingData.checkOutDate) === 1
                      ? "Night"
                      : "Nights"}
                  </p>
                </div>
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 rounded-lg border p-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {selectedBookingData.numberOfGuests || 1}{" "}
                        {selectedBookingData.numberOfGuests === 1 ? "Adult" : "Adults"}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-gray-500">Email: {selectedBookingData.guest?.email}</p>
            ...
            <div className="mb-8">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-semibold">Today</h2>
                  <p className="text-sm text-gray-500">
                    {todayBookings.length} {todayBookings.length === 1 ? "Booking" : "Bookings"} - {todayRoomCount}{" "}
                    {todayRoomCount === 1 ? "Room" : "Rooms"}
                  </p>
                </div>
                <Button
                  variant="ghost"
                  className="text-red-500 hover:text-red-600"
                  onClick={handleDownloadTodaysArrivals}
                >
                  <Printer className="mr-2 h-4 w-4" />
                  Download Today's Arrival
                </Button>
                    ...
                    {todayBookings.map((booking) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-4">
                            <div className="text-sm text-gray-500">Booking.com #{booking.id.substring(0, 8)}</div>
                          </td>
                          <td className="px-4 py-4 text-right">
                            <div className="flex justify-end gap-2">
                              ...
                              <Button className="bg-green-500 hover:bg-green-600 text-white">Checkin Now</Button>
                            </div>
                          </td>
                        </tr>
                      )
